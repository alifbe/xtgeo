...
def _compute(nc, nr, nl, permx=1.0, permy=1.0, permz=1.0, ntg=None):
    ...
    tranx, trany, tranz, nnc = grid.get_transmissibilities(px, py, pz, ntg=nt)
...
class TestNTGScaling:
    ...
    def test_ntg_none_same_as_ntg_one(self):
        ...
        tx_none, ty_none, tz_none, _ = grid.get_transmissibilities(px, py, pz, ntg=None)
        tx_one, ty_one, tz_one, _ = grid.get_transmissibilities(px, py, pz, ntg=ntg_one)
...
class TestScalarInput:
    ...
    def test_scalar_perms_run_without_error(self, grid):
        ...
        tranx, trany, tranz, _ = grid.get_transmissibilities(1.0, 1.0, 1.0)
        assert isinstance(tranx, xtgeo.GridProperty)
        assert isinstance(trany, xtgeo.GridProperty)
        assert isinstance(tranz, xtgeo.GridProperty)
    ...
    def test_scalar_ntg_run_without_error(self, grid):
        ...
        tranx, _, _, _ = grid.get_transmissibilities(1.0, 1.0, 1.0, ntg=0.5)
        assert isinstance(tranx, xtgeo.GridProperty)
    ...
    def test_scalar_perm_equivalent_to_gridproperty(self, grid):
        ...
        tx_s, ty_s, tz_s, _ = grid.get_transmissibilities(
            px_scalar, py_scalar, pz_scalar
        )
        tx_p, ty_p, tz_p, _ = grid.get_transmissibilities(px, py, pz)
    ...
    def test_scalar_ntg_equivalent_to_gridproperty(self, grid):
        ...
        tx_s, ty_s, tz_s, _ = grid.get_transmissibilities(px, py, pz, ntg=ntg_scalar)
        tx_p, ty_p, tz_p, _ = grid.get_transmissibilities(px, py, pz, ntg=ntg_prop)
    ...
    def test_scalar_ntg_scales_horizontal_tran(self, grid):
        ...
        tranx, trany, _, _ = grid.get_transmissibilities(1.0, 1.0, 1.0, ntg=0.5)
        assert np.all(
            np.isclose(
                tranx.values.filled(np.nan)[:-1, :, :], 0.5 * _C_METRIC, rtol=1e-6
            )
    ...
    def test_mixed_scalar_and_gridproperty(self, grid):
        ...
        tranx, trany, tranz, _ = grid.get_transmissibilities(px, 3.0, 4.0, ntg=0.5)
...
class TestInactiveCells:
    ...
    def test_inactive_cell_zeros_tranx(self):
        ...
        tranx, trany, tranz, _ = grid.get_transmissibilities(px, py, pz)
...
class TestPinchoutNNC:
    ...
    def test_pinchout_nnc_created(self, pinchout_grid):
        ...
        py = _uniform_prop(grid, 1.0, "permy")
        pz = _uniform_prop(grid, 1.0, "permz")
        _, _, _, nnc = grid.get_transmissibilities(px, py, pz, min_dz_pinchout=0.5)
...
class TestMinDzPinchout:
    ...
    def test_parameter_is_accepted(self):
        ...
        py = _uniform_prop(grid, 1.0, "permy")
        pz = _uniform_prop(grid, 1.0, "permz")
        _, _, _, nnc = grid.get_transmissibilities(px, py, pz, min_dz_pinchout=0.001)
        assert isinstance(nnc, pd.DataFrame)
...
class TestHandedness:
    ...
    def test_lefthanded_trany_sentinel_at_last_row(self):
        ...
        _, trany, _, _ = grid.get_transmissibilities(1.0, 1.0, 1.0)
    ...
    def test_righthanded_trany_values_match_lefthanded(self):
        ...
        _, trany_l, _, _ = grid_l.get_transmissibilities(1.0, 1.0, 1.0)
        _, trany_r, _, _ = grid_r.get_transmissibilities(1.0, 1.0, 1.0)
    ...
    def test_tranx_and_tranz_unaffected_by_handedness(self):
        ...
        tranx_l, _, tranz_l, _ = grid_l.get_transmissibilities(1.0, 1.0, 1.0)
        tranx_r, _, tranz_r, _ = grid_r.get_transmissibilities(1.0, 1.0, 1.0)
...
class TestCompareCases:
    ...
    def test_b7case_tranx_values_lh(self, testdata_path):
        ...
        # compute with xtgeo
        tranx, trany, tranz, _ = grid.get_transmissibilities(
...
class TestEmeraldOriginal:
    ...
    def test_emerald_original_nnc_vs_vendor(self, testdata_path):
        ...
        _, _, _, nncs_df = grid.get_transmissibilities(permx, permy, permz, ntg)
